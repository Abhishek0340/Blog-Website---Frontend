...
const Category = () => {
  ...
  return (
    <>
      <Helmet>
        <title>{name ? `${name} Category | Blog Website` : "Category | Blog Website"}</title>
        <meta name="description" content={name ? `Browse posts in the ${name} category on Blog Website.` : "Browse posts by category on Blog Website."} />
        <meta name="keywords" content={`category, ${name}, blog, posts`} />
        <meta property="og:title" content={name ? `${name} Category | Blog Website` : "Category | Blog Website"} />
        <meta property="og:description" content={name ? `Browse posts in the ${name} category on Blog Website.` : "Browse posts by category on Blog Website."} />
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 pb-10">
        {loading ? (
          <div>Loading...</div>
        ) : error ? (
            ...
            {filteredPosts
              ...
              .map((post, idx) => (
                <div key={post._id || idx} className="bg-white rounded-xl shadow p-4 mb-4 hover:shadow-lg transition flex flex-col">
                  ...
                  <button
                    className="mt-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition self-end"
